...
class MDPCondition(ImageSampler):
    def __init__(self,
                 unet: Any,
                 vae: Any,
                 tokenizer: Any,
                 ...
                 **kwargs):
        """
        This class defines the image sampling using mdp_condition
    ...
    def inference_step(self,
        ...
        with autocast('cuda'):
            ...
            if self.kwargs["linear_factor_list"] is None:
                all_embeddings = self.kwargs["linear_factor"] * self.kwargs[
                    "amplify"] * self.kwargs["rec_noise_embs"][index] + \
                    (1 - self.kwargs["linear_factor"]) * all_embeddings
            else:
                all_embeddings = self.kwargs["linear_factor_list"][index] * \
                    self.kwargs["amplify"] * \
                    self.kwargs["rec_noise_embs"][index] + \

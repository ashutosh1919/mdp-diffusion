...
class MDPEpsilon(ImageSampler):
    def __init__(self,
                 ...
                 **kwargs):
        """
        This class defines the image sampling using mdp_epsilon
    ...
    def test_other_param_valid(self):
        ...
        if 'rec_noise_preds' not in self.kwargs:
            raise ValueError("List param rec_noise_preds must be passed.")
        ...
        with autocast('cuda'):
            ...
            if self.kwargs["linear_factor_list"] is None:
                noise_pred = self.kwargs["linear_factor"] * self.kwargs[
                    "amplify"] * self.kwargs["rec_noise_preds"][index] + \
                    (1 - self.kwargs["linear_factor"]) * noise_pred
            else:
                noise_pred = self.kwargs["linear_factor_list"][index] * \
                    self.kwargs["amplify"] * \
                    self.kwargs["rec_noise_preds"][index] \

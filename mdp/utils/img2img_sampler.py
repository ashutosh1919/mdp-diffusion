...
class Img2ImgSampler(ImageSampler):
    def __init__(self,
                 unet: Any,
                 vae: Any,
                 tokenizer: Any,
                 ...
                 **kwargs):
        """
        This class defines the image sampling based on image
    ...
    def test_other_param_valid(self):
        if self.latent is None:
            raise ValueError(
                "Latent tensor must not be None for Img2ImgSampler")
        if self.generator is not None:
            print("WARN: generator is ignored for Img2ImgSampler")
